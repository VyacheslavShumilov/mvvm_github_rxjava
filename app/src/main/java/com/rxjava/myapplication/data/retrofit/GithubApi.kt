...
// Принцип работы rx java в данном примере -> в rx java есть объект "Observable" (наблюдаемый), который испускает событие (за нас это делает retrofit) и есть observer (наблюдатели),...
// ...которые будут эти собитыя потреблять и смотреть, что произошло

interface GithubApi {
    @GET("users")
    fun getUsers(): Single<List<UsersEntity>>     // для перехода на rx java заменяем Call<List<...>> на Single<List<...>> (вместо возвращения Пользователя, возвращаем Single)
}

// Существуют разные объекты:
